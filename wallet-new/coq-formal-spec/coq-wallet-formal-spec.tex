...
\usepackage{amsmath, amssymb, amsmath, stmaryrd, latexsym, amsthm, mathtools}
...
\usepackage{verbatim}
%C:\Users\Polina\AppData\Local\Temp\mik81306
\usepackage{hyperref}
\hypersetup{
  colorlinks=false,
  linkcolor={blue},
...
\subsection{Formalizing UTxO-style Accounting}
...
We begin our discussion by outlining the logic and axioms we have used in this 
formalization. \newline

\noindent {\bfseries Classical vs. Intuitionistic Logic.} 
It does not appear necessary to introduce a classical
logic axiom such as the law of excluded middle into our formalization, thus restricting
our reasoning to the Coq default set of intuitionistic axioms. We may introduce a 
classical logic an axiom if needed in future development. \newline

\noindent {\bfseries Decidable Equality.} Given that we have chosen to
 use lists to represent the subsets and finite maps within the derived types, we 
 require that equality on the primitive types be decidable in order to be able to perform some of the necessary computations on the terms. 
 
 This type of equality map may be required to, for example, filter a list or construct a decidable version of \texttt{In}. Note that decidable boolean equality deals with sets, i.e. \texttt{bool : Set}, whereas classical logic is relevant only for \texttt{Prop}.
 This means that equality decidability is not automatically introduced by the law of
 excluded middle (or other related classical axioms).
 
 We have added axioms which state equality is decidable on some of the primitive types,
 and will give the details as we introduce the types below, as well as discuss the
  details for the equality predicates on the derived types. \newline
  
  
\noindent {\bfseries Proof Irrelevance.} As we will demonstrate later, for some of the derived types, we 
considered it useful to aggregate the value of the term with a proof (that this value 
satisfies a particular property we require) into a single structure representing the
corresponding informal type. In order to 
make equality judgments between such terms, we are only interested in comparing the
values of the terms themselves, not the associated proofs that these must satisfy. For
this reason, we have inserted the proof irrelevance axiom into our code: \newline

 \texttt{Axiom proof\_irrelevance : forall (A : Prop) (a1 a2 : A), a1 = a2.} \newline

This axiom identifies all proofs of the same proposition as equal. \newline

\noindent {\bfseries Well-Definedness.} We remind the reader that a well-defined map $f : A \to B$ is one with a single output for a given input, i.e. 
\[a = b \Rightarrow f(a) = f(b)\] 

The well-definedness condition is, in fact, required in order to be able to prove the
basic properties of UTxO operations both in the case of using predicates to represent
subsets and in. For this reason we must build this property into the structures we use
 to represent finite map types. \newline

\noindent {\bfseries Primitive Types.} We first explain the design decisions for the 
formalization of the primitive types. \newline

\texttt{TxId}: We have chosen to avoid including this type in the formalization 
entirely. The \texttt{txid : TxId} value is obtained by applying a hash function to a 
transaction \texttt{tx : Tx}. Given the context of the transaction \texttt{tx} (i.e. 
the relevant blockchain information), it is indeed possible to find the transaction with 
the hash value \texttt{txid}, in fact, this operation is relied upon by computations 
discussed in later chapters. However, if we consider the possibility of defining
an invertible map \texttt{get\_id : Tx $\to$ TxId}, we may encounter a problem.

In the Coq formal specification of the wallet, we will not be able to instantiate 
the hash map and it's contextual inverse with actual computations (at least not 
without straying very far from the intended goal of this formalization). For this
reason, we were hesitant to add an axiom saying that we have maps in both
directions between \texttt{Tx} and \texttt{TxId} that are mutual inverses.

We have also considered a Coq Module-based approach to eliminating the need for such
an axiom, but, again, there is no sensible way to instantiate the module with data that
would simulate the contextual retrieval of the correct \texttt{tx} using its hash.

Furthermore, abolishing \texttt{TxId} for the purposes of this formalization eliminates
the need for defining \texttt{Tx} and \texttt{TxId} as mutually recursive types,
simplifying the wallet specification without losing information. We have
instead chosen to refer to the particular \texttt{Tx} directly everywhere that 
\texttt{TxId} is used in the original formal specification. \newline

\texttt{Ix}: This primitive type is a variable. The decidability of the equality
predicate is added as an axiom into the code.\newline

\texttt{Addr}: This primitive type is a variable. The decidability of the equality
predicate is added as an axiom into the code. \newline

\texttt{Coin}: This primitive type is defined to be \texttt{nat} in our formalization
 in order to perform the necessary calculations. The standard boolean equality 
 computation \texttt{eq\_nat} is used. \newline
 
\noindent {\bfseries Derived Types.} Next, we discuss the derived types. In order to represent 
derived types, it is first necessary to decide on the formalism for the notions of a
 subset and a finite map. We considered the following options: \newline
 
 \noindent \underline{Subsets.} In Coq, subsets of a set \texttt{A : Set} can be most easily
 represented using predicates on \texttt{A}. That is, each subset can be defined
 using a map of the form \texttt{P : A $\to$ Prop}, where \texttt{P a} is provable whenever \texttt{a} is contained in the subset which \texttt{P} defines, and \texttt{False} otherwise. Each subset can be directly referred to by the predicate
 that defines its membership, e.g. a set of addresses satisfying some property \texttt{P : Addr $\to$ Prop} would be passed as a parameter simply as \texttt{P}. This
 approach is a very convenient way to reason about sets, especially since for many of
 the proofs in the wallet formal specification, the Coq proposition prover tactic
 \texttt{tauto} can be used to do most of the work for us. 
 
 Additionally, in order to make meaningful equality comparisons between subsets \texttt{P, P' : A $\to$ Prop},
 it is necessary to introduce both the functional extensionality and the propositional extensionality axioms.
 
 Finite maps, as they are,
 in essence, sets of pairs of the form $(a, f(a))$, can also be presented in this way. We formalize a finite map $f : A \to B$ by a relation \texttt{P : A*B $\to$ Prop}.
 
 This approach, however, comes with one unassailable caveat: for any calculations involving iterating over all members of the set or finite map (such as calculating \texttt{balance}, etc.) there
 is no obvious way to model this iteration in the world of predicate-defined
 sets or finite maps, either inductively nor recursively. 
 
 In a clever effort to remedy this, we have attempted to introduce additional structure
  into the predicate-defined
 finite maps by adding a special parameter to the header of certain calculations which
 require iteration over all pairs of the given finite map.
 More specifically, this parameter is a proof that the given finite map is well-defined,
 where the well-definedness predicate is given inductively.

When representing finite maps using predicates, the property of well-definedness
stated above would take the form  

\[ \texttt{forall a b c, (P (a,b)) $\wedge$ (P (a,c)) $\to$ b = c} \]

 We can write an inductive definition of this property in the following way: \newline

\texttt{ \indent Inductive wd (A B : Type) : (FinMap A B) $\to$ Type := \\
\indent \indent \indent | wd0 : wd A B (null\_fm A B) \\
\indent \indent \indent | wd\_new : forall a b f, wd A B f $\to$ not\_in\_dom a f \\
\indent \indent \indent \indent $\to$ wd A B (insert a b f). } \newline

\noindent where \texttt{null\_fm} is the empty finite map, and \texttt{not\_in\_dom} 
is true when \texttt{a} is not in the domain of the finite map \texttt{f}. So, \texttt{f}
is well-defined whenever it's either empty or built by inserting a point map $a \to f(a)$ into an existing w.d. map that does not already contain $a$ in its domain. 

Note that the type of \texttt{wd} is a map into \texttt{Type}, rather than
\texttt{Prop}. The reason for this is that this way, when we require that a map be 
w.d., we are able to use an argument
of type \texttt{wd TxIn TxOut f} to define fixpoint calculations needed for the wallet,
e.g. the sum of the coin values of outputs of the finite map:
\newline

\texttt{ \indent Fixpoint sum\_fm (f : UTxO) (f\_wd : wd TxIn TxOut f) : nat := \\
\indent \indent \indent match f\_wd with \\
\indent \indent \indent | wd0 \_ \_ $\Rightarrow$ 0 \\
\indent \indent \indent | wd\_new \_ \_ \_ b f' wdf' \_ $\Rightarrow$ (snd b) + (sum\_fm f' wdf') \\
\indent \indent \indent end. } \newline

Now, the problem with this approach is that we can no longer make use of proof 
irrelevance because \texttt{wd \_ f} has type \texttt{Type}. That is, we cannot
show that \newline

\indent \indent \texttt{ forall utxo f\_wd f\_wd', sum\_fm utxo f\_wd = sum\_fm utxo f\_wd'} \newline

\noindent using the \texttt{proof\_irrelevance} axiom. Independence of the result of the
sum calculation from the specific proof that a given UTxO is w.d. is a property that will be required for completing further proofs about the wallet. Furthermore, this
property cannot be axiomatized without introducing an inconsistency.

We did not find any other promising ways to build a term that connects the predicate
definition of a particular subset with an inductive (or recursive) step required
for the wallet calculations. Thus, we decided to instead consider lists for our 
formalization. \newline

\noindent \underline{ Lists and List-Based Libraries.} Lists appear to be the better
option for building the wallet formalization, and model the actual
 implementation of the wallet somewhat more closely. There are several libraries we  
 have considered using that are built on lists, but the additional features these
 offered were not the right selections for the purpose of wallet formalization, so we
 proceeded to work with lists directly.
 
 For example, we considered using the \texttt{FSets} library (which includes finite maps \texttt{FSets.FMapList}). This library requires defining an order on the terms
 in the list in order to build a finite map. Because some of the types we are working
 with (e.g. \texttt{Addr}) are set as variables, most of the ordering within the
 finite maps we build using this library is simply axiomatic, and does not introduce any additional structure
 to the resulting representation. There is also no reason behind maintaining this order, 
 nor does ordering the list more closely resemble the wallet implementation.
  
 Thus, we decided that formally, the power set $\mathbb{P} (A)$ and the finite map 
 $A \mapsto B$ will be represented by
 \newline
 
 \indent \indent \texttt{Definition Pow (A: Type) := list A.} \\
 \indent \indent \texttt{Definition FinMap (A B : Type) := Pow (A*B).} \newline
 
 As mentioned above, we must additionally ensure that any finite map we define is
 well-defined. In order to do this, we have
 decided to use Coq's type classes. Type classes are a very powerful and versatile
 tool for aggregating terms and proofs into a compound concept, and efficiently 
 manipulating the resulting structure. 
 
 The uses cases of type classes are typically 
 more complex notions, formalizing limits in category theory being a very representative
 one. In our case, the complexity of the structure we wish to formalize
 (one term together with a proof of one property of it) may not justify the use of type classes,
 however, they do also provide a wider range of useful features than
 using structures or sigma types. 
 
 The well-defined finite map type class we have 
 built takes two arguments, the input and the output type of the finite map. Inside,
 it contains two terms which must be defined in order to instantiate it (the finite
 map and a proof that it is well-defined). \newline
 
 \indent \indent {\ttfamily Class FinMapWD (I O : Type) : Type := \\
 \indent \indent \{ \\
 \indent \indent \indent 	fm : FinMap I O ; \\
 \indent \indent \indent	ins\_unique : ins\_unique\_f I O fm \\
 \indent \indent \}.} \newline
 
 Here, the \texttt{ins\_unique} term is a proof that each input is unique in the pairs
 in \texttt{fm}, i.e. \texttt{NoDup (fst (split fm))}. This condition implies
 well-definedness, and is an efficient way to not only ensure that, but also to avoid
 having any duplicate entries in the finite map list. 
 
 In order to make use of these in our formalization, we must also discuss the equality
 comparison map on these types. In line with informal set equality, 
 
  \[ S_1 = S_2 \Leftrightarrow (S_1 \subseteq S_2) \wedge (S_2 \subseteq S_1) \]
  
 \noindent equality of finite maps and power sets in our formalization is defined in 
 terms of double inclusion. This is an equivalence relation, however, it is still 
 necessary to prove, for each individual case, that a rewrite is permitted. For example,
given \texttt{o1 o2 : Pow TxOut}, if these are equal in the double inclusion sense,
we would still need to prove that \texttt{fst (split o1)} and \texttt{fst (split o2)}
 are also equal. We have introduced lemmas for each such case where necessary.
 
 For the case of equality of finite maps (here we will proceed with \texttt{UTxO} as
 a representative example), we cannot directly use double inclusion as a way to 
 make an equality judgment. What we do instead is use the double inclusion criteria
 for the underlying list of pairs, and denote this predicate \texttt{UTxOeq}.
 
 Now we discuss the specifics of formalization of the derived
 types.

\texttt{TxOut}: This definition is entirely consistent with the informal one, i.e. 
\newline

\indent \indent \texttt{TxOut := Addr*Coin} \newline

\texttt{TxIn}: Having done away with \texttt{TxId}, this definition becomes \newline
 
 \indent \indent \texttt{TxIn := Tx*Ix} \indent and \\
 \indent \indent \texttt{Ins := Pow TxIn} \newline
 
The type \texttt{Tx} is defined below. \newline
 
\texttt{UTxO}: The type class \texttt{UTxO} we have defined using our \texttt{FinMapWD}
type class, \newline

\indent \indent \texttt{UTxO := FinMapWD TxIn TxOut} \newline

Note that there is nothing surprising or potentially mismatched with the reality of the
UTxO implementation - each new transaction must have a new ID, therefore the \texttt{ins}
of the UTxO will in reality always be non-duplicate.
 
\texttt{Tx}: To define this type, we must again make use of the \texttt{FinMapWD} type
class. We define the finite map \newline

\indent \indent \texttt{IxTxOut := FinMapWD Ix TxOut} \newline

Here, again, the non-duplicatation of \texttt{Ix} in a given context is expected in the 
implementation as well. 

We now define the term \texttt{Tx} representing the informally-presented type of \texttt{Tx}. Recall that here, we use \texttt{Tx} itself instead of \texttt{TxId}, which means
 \texttt{Tx} must be defined inductively. \newline
 
 \indent \indent \texttt{Inductive Tx := MkTx : Pow (Tx * Ix) $\to$ IxTxOut $\to$ Tx.} \newline
 
 Note that one constructor here is sufficient, since it already takes into account the base case \texttt{MkTx nil nil}. Note also that it appears that an additional constraint on the construction
 of \texttt{Tx} is necessary - more on this in the Invariants section, \ref{sec:invariants}.
 
 \texttt{Block := Pow Tx.} It appears that in order to complete the invariant proofs, the terms of this
type must be paired with a proof that for any two \texttt{tx1 tx2 : Tx} the set \texttt{ins1} of 
\texttt{tx1} is entirely disjoint from set \texttt{ins2} of 
\texttt{tx2}
 
 \texttt{Pending := Pow Tx.} The same additional constraint on this type is required as for 
 \texttt{Block} above. \newline
 
 \texttt{ours : Addr $\to$ bool} is a variable. \newline
 
 \texttt{Fixpoint TxOuts\_ours (outs : Pow TxOut) : Pow TxOut.} \newline 
 
 Is a term which filters a set of TxOuts using the \texttt{ours} filter. Note that
 this is slightly different than the informal definition, in that we cannot, in the Coq
 code, quantify over all possible addresses and filter them using \texttt{ours}. So 
 instead, our formalized \texttt{TxOuts\_ours} term takes a set of TxOuts as an
 argument. It is indeed the case that we are always filtering some \textit{specific}
  set of TxOuts when we make use of \texttt{TxOuts\_ours}. We will elabourate on this
 below as we introduce the first use of this term.


\subsection{Operations on UTxO}

The usual set operations, such as union and intersection, can be performed on arbitrary
sets of inputs (i.e. terms of type \texttt{Ins}). As for UTxOs, there is no need
to take intersection within the scope of this formal specification. However, when taking the
union of two UTxOs, we must additionally require that the inputs of these two finite
maps are disjoint. That is, the union operation takes an additional proof parameter,
so that the header becomes \newline

\indent \indent \texttt{Definition unionUTxO (u v : UTxO) \\
\indent \indent \indent	(pf : \# iu ((\# u)++(\# v))) : UTxO} \newline

\noindent Here, the notation \texttt{\# \_} represents the underlying finite map that makes up
the UTxO, and \texttt{\# iu \_} is the proposition stating the inputs of the UTxO
are all distinct.

For the properties in this and subsequent chapters, a \texttt{pf} argument (as above) is required whenever a
 union of UTxOs is taken. This is because a term of type \texttt{UTxO} is an instance of a
type class, building which requires defining a finite map and proving that it is indeed
well defined (the term \texttt{pf} is exactly the required proof). Note that taking the
union of two sets represented by lists is simply the \texttt{app} (or $++$) operation.

Next, we discuss the intersection and exclusion of \texttt{Ins} lists. These are the 
usual filtering operations, however, these operations require assuming decidability of
equality on \texttt{TxIn}. This decidability property can be proved directly if all the
relevant types are instantiated (as opposed to set as variable).

The domain is defined by \newline 

\indent \indent \texttt{Definition dom (u : UTxO) := fst (split (\# u)).} \newline

Now, for the following operations used to filter UTxOs (with the headers of the 
corresponding terms in our code given below each),
%
\begin{align*}
...
\indent \indent \texttt{Definition d\_r (ins : Ins) (u : UTxO) : UTxO.} \\
\indent \indent \texttt{Definition d\_e (ins : Ins) (u : UTxO) : UTxO.} \\
\indent \indent \texttt{Definition r\_r (u : UTxO) (outs : Outs): UTxO.} \\

All three of these take two parameters and build a new filtered UTxO. To make this 
UTxO, we first build a list of pairs (finite map). In our 
formalization, we additionally have lemmas demonstrating that the inputs of the resulting 
filtered finite map are all still distinct. We can use these to make 
the UTxO the operation calls for.

When it comes to applying \texttt{r\_r} to a UTxO and the set TxOut$_{\text{ours}}$,
we cannot formally do this in our code, as mentioned before. For this reason we defined
a special case of the range restriction term for restricting a UTxO's range to only the
 \texttt{ours} addresses,
\newline

\indent \indent \texttt{Definition r\_r\_ours (u : UTxO) : UTxO.} \newline

\noindent and here, again, the definition consists of a finite map and a 
well-definedness proof. 

We have defined some small sanity check examples which include a calculation of
the domain restriction of a five-entry UTxO to a list of three inputs, and it behaves
correctly. Note that this is the way the well-definedness issue originally came to light: when
we built the initial predicate-based formalization, we were not able to should that
a domain-restricted UTxO contains the entries we informally expect it to. Having moved
to formalizing using lists instead, we were still not able to achieve the expected
behaviour without a proof of well-definedness as one of the parameters.

We have formally proved the properties in the following lemma: 
...
We give an example of the formal formulation of the 4th property in the lemma: \newline

\indent \indent {\ttfamily Definition UTxO\_prop4 : forall (ins : Ins) (u v : UTxO) \\
\indent \indent	\indent  (pf : \# iu ((\# u)++(\# v))),  \\
\indent \indent	\indent UTxOeq (d\_r ins (unionUTxO u v pf)) (unionUTxO (d\_r ins u) (d\_r ins v) \\
\indent \indent \indent	(@sub\_utxo\_nodup (d\_r ins u) u (d\_r ins v) v \\
\indent \indent \indent 			(UTxO\_prop1 ins u) (UTxO\_prop1 ins v) pf)).
	} \newline 

Here, \texttt{@sub\_utxo\_nodup ...} is a proof term required as an argument when
taking the union of two UTxOs (\texttt{(d\_r ins u)} and \texttt{ (d\_r ins v)}), which 
is constructed making use of a general proof of the fact that subsets of inputs of two
UTxOs with disjoint subsets of inputs are disjoint. 

The proofs of the above lemmas are significantly different from the informal set-theoretic ones, as they are done by induction on UTxOs and case analysis. They are, however, very
susceptible to automation, and for the most part are made up of steps such as applying
the inductions hypothesis and some auxiliary lemmas. We also make significant use of the
\texttt{ins\_unique} term in the proofs. 

Now let us consider the two preorders on UTxOs:
...
in the UTxO, rather than the individual outputs. Given UTxOs \texttt{u} and \texttt{v},
the second definition can be expressed as: \newline

\indent \indent \texttt{ Definition S\_UTxOinc (u v : UTxO) : Prop := \\
\indent \indent \indent (UTxOinc u v) $\wedge$ \\
\indent \indent \indent (forall (io io':(TxIn*TxOut)), In io (\# u) $\to$ In io' (\# v) \\
\indent \indent \indent $\to$ (fst (fst io) = fst (fst io')) $\to$ In io' (\# u)).} \newline



\subsection{Other auxiliary operations}

We have formalized the necessary auxiliary operations:
%
\begin{align*}
...
The map \texttt{txins} iterates through a list of \texttt{Tx}, and for each transaction
given by \texttt{MkTx i o},
it appends the \texttt{i : Ins} of that transaction to the output, i.e. \newline

In order to define the \texttt{txouts}, we again need to make use of pattern matching.
We define a term \newline

\indent \indent Fixpoint txouts\_calc (txs : Pow Tx) : Pow (TxIn*TxOut) \newline

\noindent to construct the finite map underlying the
structure \texttt{txouts} builds. For each \texttt{tx} in the input list \texttt{txs} of transactions,
it appends the list \texttt{makeUTxOforTx tx o} to the output finite map, where
\texttt{o} is the outputs of \texttt{tx}, i.e. the \texttt{o} in \texttt{MkTx l o}.
We define this term in the following way: \newline

\indent \indent \texttt{Fixpoint makeUTxOforTx (tx : Tx) (outs : FinMap Ix TxOut) : FinMap TxIn TxOut := \\
\indent \indent \indent  match outs with \\
\indent \indent \indent  | nil $\Rightarrow$ emptyUTxO \\
\indent \indent \indent  | o :: l $\Rightarrow$ ((tx, fst o), (snd o)) :: makeUTxOforTx tx l \\
\indent \indent \indent  end.} \newline

Note that here, we pass the \texttt{tx} term as a parameter in addition to the outputs of this
transaction. The reason for this is that we build a finite map entry pair to add to the final
resulting UTxO using the transaction \texttt{tx} directly, so we cannot change the outputs within this 
parameter. In the original wallet spec document, this would actually be \texttt{TxId} instead of \texttt{Tx}.

In order to obtain the desired UTxO, \texttt{txouts} is applied to a list of transactions. 
The finite map part of the UTxO is build by the \texttt{txouts\_calc} term described above.
In order to build an instance of the \texttt{UTxO} type class using this finite map, we must
also prove that the resulting finite map's inputs are all distinct. For a particular \texttt{tx},
it is straightforward to prove that \texttt{makeUTxOforTx tx (\# outs)} in indeed w.d., but
additional constraints need to be placed on a list of \texttt{Tx} in order to show that the 
output of \texttt{txouts\_calc} is w.d. 

One option (currently the one followed in the code)
is to express this constraint directly (as a proof parameter in the \texttt{txouts} definition):
\newline

\indent \indent \texttt{txouts\_unique : ins\_unique\_f (txouts\_calc txs)} \newline

\noindent and also add this type of parameter as needed within subsequent term definitions.
Another possibility is to impose a constraint on the \texttt{txs : list Tx} directly, and use
the w.d. of \texttt{makeUTxOforTx tx (\# outs)} for each \texttt{tx} in combination wtih this
constraint to prove the same thing \texttt{txouts\_unique} is proving directly. It is not 
immediately clear if there are advantages to this approach as it relates to the implementation
of the wallet (or for other reasons).

The definition of the \texttt{balance} term matches the informal definition exactly. We have also formalized the following lemmas and definitions:

\begin{definition}[Dependence : \texttt{depends\_on}.] 
...
\begin{definition}[Set of independent transactions : \texttt{independent}.] 
...
other operators, proved by lemmas with corresponding numbers in our code, \texttt{lemma161}
and \texttt{lemma162}.
%
...
%
\begin{itemize}
\item inquire as to the balance of the wallet (total balance and
      available balance).
...
In our formalization, the type of the wallet is \texttt{UTxO*Pending}, and so an empty
wallet is given by \newline

\indent \indent \texttt{Definition empty\_w : Wallet := (emptyUTxO, @nil Tx).} \newline

For the some of the definitions of queries, updates, and auxiliary functions, we had to
 introduce an additional parameter in order to maintain the w.d. nature of the wallet UTxO. 
For the terms in which a union of UTxO's is taken, a proof that the two UTxO's inputs
are distinct is necessary, consider for example:  \newline
 
 \indent \indent \texttt{Definition total (w : Wallet) \\
 \indent \indent \indent (txouts\_unique : ins\_unique\_f (txouts\_calc (snd w))) pf : UTxO := \\
 \indent \indent \indent unionUTxO (available w) (change (snd w) txouts\_unique) pf.} \newline
 
 The term \texttt{pf} is a proof that the \texttt{available w} and \texttt{change (snd w)}
 UTxOs have disjoint sets of inputs, needed to be able to take their union. Note also the 
 term \texttt{txouts\_unique}, which is a necessary parameter whenever the \texttt{txouts}
 term is called on in the body of the definition. This term is a proof that the UTxO
 build by the calculation within \texttt{txouts} is w.d. This cannot be proved from the 
 definition of the calculation without imposing \textit{some} constraint on the list 
 \texttt{txs : Pow Tx} passed as a parameter to \texttt{txouts\_calc}. We chose to go
 the most direct route and add \texttt{txouts\_unique} to serve this purpose.
 
We added the parameter \texttt{txouts\_unique} (as above) to the following terms:
\texttt{change, total, updateUTxO, applyBlock} and \texttt{totalBalance} in 
\cref{fig:wallet_interface}. We also added the parameter \texttt{pf} to the same terms,
except \texttt{change}, which does not require taking a union of UTxOs.

It is worth noting that when formalizing \texttt{updateUTxO}, the range restriction of
the output of \texttt{txouts} to \texttt{TxOut\_ours} is done, as discussed earlier, 
by applying the specially-defined range restriction term that filter an input UTxO
to only our addresses, so that the body of the term becomes \newline

\indent \indent \texttt{d\_e (txins b) (unionUTxO utxo (r\_r\_ours (txouts b txouts\_unique)) pf)} \newline

The rest of the formalization is essentially consistent with \cref{fig:wallet_interface}.  
...
This lemma is formalized and proved as \texttt{lemma33} in our code. Note that the lemmas
and invariants are numbered in our code according to the original wallet spec document, 
rather than this one (this will be addressed when the numbering in this document is no
longer expected to be updated).

\subsection{Invariants}
\label{sec:invariants}

In order to prove the invariants discussed below, we inductively construct a predicate on the 
\texttt{Wallet} type. This predicate is true whenever a \texttt{w : Wallet} is constructed 
inductively via only $w_\emptyset$, $\mathsf{applyBlock}$ and $\mathsf{newPending}$: \newline


\indent \indent \indent \texttt{Inductive atomic\_updates\_rel : Wallet $\to$ Prop := \\
\indent \indent \indent | is\_em : atomic\_updates\_rel empty\_w \\
\indent \indent \indent | app\_b : forall b w (t\_u : ins\_unique\_f (txouts\_calc b)) pf precond, \\ 
\indent \indent \indent \indent atomic\_updates\_rel w $\to$ atomic\_updates\_rel (applyBlock b w t\_u pf precond) \\
\indent \indent \indent | new\_pen : forall tx w precond, atomic\_updates\_rel w $\to$ \\
\indent \indent \indent \indent atomic\_updates\_rel (newPending tx w precond).} \newline

The following invariants should hold true for a \texttt{w : Wallet} such that 
\texttt{atomic\_updates\_rel w}.
...
We have formalized the invariant as follows (however, we omit the discussion of the proof of 
this invariant for now) :

\indent \indent {\ttfamily Lemma invariant34 : forall w (at\_up : atomic\_updates\_rel w), \\
\indent \indent \indent incl (txins (snd w)) (dom (fst w)).} \newline
...
\end{invariant}
 ...
 When proving the above invariant, the strategy we followed is to begin the proof by structural
 induction on the proof \texttt{at\_up : atomic\_updates\_rel w}. For the second case, 
 (i.e. \texttt{applyBlock}), we then proceed by induction on \texttt{b: Block}. The 
 equality described above seems to be required for the proof to be constructed. \newline
